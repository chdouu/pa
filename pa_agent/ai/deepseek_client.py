...
def _provider_max_output_tokens(settings: AIProviderSettings) -> int:
    ...
    elif "generativelanguage.googleapis.com" in settings.base_url.lower() and model.startswith("gemini-"):
        cap = 65_536
    else:
...
def _resolve_thinking_params(
    ...
    *,
    ...
    if "generativelanguage.googleapis.com" in settings.base_url.lower() and model.startswith("gemini-"):
        # Google's OpenAI endpoint maps reasoning_effort to Gemini thinking.
        # Gemini 2.5 Flash can disable thinking; Gemini 3+ uses its default when unset.
        if not _thinking:
            return {}, "none" if model.startswith("gemini-2.5-flash") else None
        return {}, "high" if _effort == "max" else (_effort or "high")
    ...
    def chat(
        ...
        *,
        ...
        client = _OpenAI(
            ...
            **({"max_retries": self._max_retries} if self._max_retries is not None else {}),
        )
        ...
        if not _thinking_on and "generativelanguage.googleapis.com" not in self._settings.base_url.lower():
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                # Only repeat a request when the provider explicitly rejects this option.
                error_text = str(exc).lower()
                if "stream_options" not in error_text or getattr(exc, "status_code", None) not in (400, 422):
                    raise
