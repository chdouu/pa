...
GEMINI_FALLBACK_MODELS: tuple[tuple[str, str], ...] = (
...
def default_fallback_models() -> list[FallbackModel]:
    return [FallbackModel(name=name, model_id=model_id) for name, model_id in GEMINI_FALLBACK_MODELS]


class FallbackAPIGroup(BaseModel):
    name: str = "Gemini API"
...
class AIProviderSettings(BaseModel):
    ...
    fallback_groups: list[FallbackAPIGroup] = Field(default_factory=list)
...
def provider_api_key_configured(settings: Settings | None) -> bool:
    ...
    if provider.fallback_enabled:
        return any(group.enabled and group.api_key.strip() and any(
            model.enabled and model.model_id.strip() for model in group.models
        ) for group in provider.fallback_groups)
