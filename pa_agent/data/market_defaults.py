...
def normalize_gold_symbol_for_kind(kind: str, symbol: str) -> str:
    ...
    if kind == "tradingview":
        ...
        if _is_hk_tv_code(hk):
            return hk
    if not sym or (kind != "tradingview" and is_likely_crypto_symbol(sym)):
        return GOLD_TV_SYMBOL if kind == "tradingview" else GOLD_MT5_SYMBOL
    if kind == "tradingview" and sym.lower() == "xauusdm":
        return GOLD_TV_SYMBOL
    return sym


def normalize_gold_tv_exchange(exchange: str) -> str:
    """Normalize a persisted TV exchange without discarding custom venues."""
...
def migrate_general_gold_defaults(general: dict) -> None:
    """Normalize legacy defaults while keeping explicit TV exchange choices."""
    ...
    if (
        kind == "tradingview"
        and ex == "TVC"
        and general["last_symbol"].upper() == "XAUUSD"
    ):
        general["last_symbol"] = "GOLD"
