...
class TradingViewSource(DataSource):
    ...
    def __init__(self, username: str = "", password: str = "",
                 extended_session: bool = False) -> None:
        self._username = username
        self._password = password
        self._tv = None          # tvDatafeed instance
        self._extended_session = extended_session
        ...
        for attempt in range(1, _TV_FETCH_RETRIES + 1):
            try:
                ...
                if self._extended_session:
                    request["extended_session"] = True
                df = self._tv.get_hist(**request)
...
def _row_ts_ms(row) -> int:
    ...
    stamp = pd.Timestamp(getattr(row, "datetime", None))
    if stamp.tz is None:
        # tvDatafeed emits host-local wall time without a timezone. Its UNIX
        # timestamp must be recovered before comparing it with watch's UTC bar.
        stamp = stamp.tz_localize(get_localzone(), ambiguous="raise", nonexistent="raise")
    return datetime_to_ts_ms(stamp)
