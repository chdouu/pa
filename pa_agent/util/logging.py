...
class MaskingFormatter(logging.Formatter):
    ...
    def __init__(self, fmt: str, api_key: str = "") -> None:
        ...
        self._api_keys: set[str] = {api_key} if api_key else set()

    def format(self, record: logging.LogRecord) -> str:  # noqa: A003
        message = super().format(record)
        for key in sorted(self._api_keys, key=len, reverse=True):
            message = message.replace(key, mask_secret(key))
        return message
    ...
    def set_api_keys(self, keys: list[str]) -> None:
        self._api_keys = {key for key in keys if key}
...
def update_api_keys(keys: list[str]) -> None:
    """Replace the set of provider keys masked in every log handler."""
    for formatter in _active_formatters:
        formatter.set_api_keys(keys)
