...
def test_stage2_includes_exchange_backed_active_thesis(assembler: PromptAssembler):
    frame = _make_frame()
    messages = assembler.build_stage2(
        frame,
        {},
    ...
    assert "OKX Active Thesis" in prompt
    assert '"current_tp1": "81432"' in prompt
    assert "lower_tp1" in prompt
    assert "raise_tp2" in prompt
