...
def test_normalize_stage2_defaults_tp_update_to_none() -> None:
    out = normalize_stage2({"decision": {"order_type": "不下单"}})
    decision = out["decision"]
    assert decision["tp_update_action"] == "none"
    assert decision.get("proposed_take_profit_price") is None
    assert decision.get("proposed_take_profit_price_2") is None


def test_normalize_stage2_keeps_only_selected_tp_proposal() -> None:
    out = normalize_stage2({
        "decision": {
            "order_type": "不下单",
            "tp_update_action": "lower_tp1",
            "proposed_take_profit_price": 105,
            "proposed_take_profit_price_2": 130,
        }
    })
    assert out["decision"]["proposed_take_profit_price"] == 105
    assert out["decision"]["proposed_take_profit_price_2"] is None
